...
def get_batch(batch_indices, data, device):
    ...
    start_pos_ids, end_pos_ids = get_position_ids(start_pos, end_pos, max_length)
    note_flag = torch.tensor(note_flag).long()
    ...
    return padded_sentence_ids.to(device), \
           ...
           context, \
           eve_type, \
           start_pos_ids.to(device), \
           end_pos_ids.to(device), \
...
def get_position_ids(batch_start_pos, batch_end_pos, max_length):
    ...
    return start_pos_ids.float(), end_pos_ids.float()
