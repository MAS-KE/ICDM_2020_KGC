...
class Process():
    ...
    def load_cache(self, mode='train'):
        dataPath = os.path.join(self.data_root, '{}_data.pkl'.format(mode))
        if os.path.exists(dataPath):
            print('Loading {} Cache ...'.format(mode))
    ...
    def process(self, mode='train', filePath='train_data.json'):
        all_data, status = self.load_cache(mode)
        if status:
            return all_data
        all_data = []
        for instance in tqdm(json.load(open(os.path.join(filePath), 'r', encoding='utf-8'))):
            ...
            reason_list = instance['reason']
            reason_info = self.data_check(reason_list, context)
            for eve_type, query in self.event_query_map.items():
                query_context = query.format(product_brand).lower()
                # query_context += '# {} # '.format(eve_type)
                ...
                note_flag = 0
                start_pos, end_pos = [], []
                for reason in reason_info:
                    if reason['event_type'] == eve_type:
                        note_flag = 1
                        start_pos.append(reason['reason_start'] + query_len + 2)
                        end_pos.append(reason['reason_end'] + query_len + 2 - 1)
        ...
        with open(os.path.join(self.data_root, '{}_data.pkl'.format(mode)), 'wb') as f:
